...
def make_CA_graph(CA_data_files, data_dirs, Labels):
    encodings = ['utf-8', 'latin1', 'ISO-8859-1', 'utf-16']  # Add more encodings if needed
    labels = Labels
    plt.figure(figsize=(10, 6))
...
def porovnej_IVC(file_paths,labels):
    ...
    output_path = os.path.join(output_dir, "ivc_comparison_for_GaIn_Si_samples.png")
...
data_dirs = [
    r'G:\Můj disk\stáž\Si - GaIn September 19 2024\GaIn_Si-n type\data txt',
    r'G:\Můj disk\stáž\Si - GaIn September 19 2024\GaIn_Si-n type no Cu tape\data txt',
    r'G:\Můj disk\stáž\Si - GaIn September 19 2024\GaIn_Si-p type\data txt',
    r'G:\Můj disk\stáž\Si - GaIn September 19 2024\GaIn_Si-p type no Cu tape\data txt',
]


SLOŽKY_K_POROVNÁNÍ = ['GaIn_Si-n type_05_CV_C01', 'GaIn_Si-n type_05_CV_C01','GaIn_Si-p type_05_CV_C01','GaIn_Si-p type_05_CV_C01']
file_paths = [rf'{data_dirs[0]}\{SLOŽKY_K_POROVNÁNÍ[0]}.txt', rf'{data_dirs[1]}\{SLOŽKY_K_POROVNÁNÍ[1]}.txt',rf'{data_dirs[2]}\{SLOŽKY_K_POROVNÁNÍ[2]}.txt',rf'{data_dirs[3]}\{SLOŽKY_K_POROVNÁNÍ[3]}.txt']
labels = [r'$\mathrm{GaIn}$/Si n-type', r'$\mathrm{GaIn}$/Si n-type (Cu tape)',r'$\mathrm{GaIn}$/Si p-type', r'$\mathrm{GaIn}$/Si p-type (Cu tape)']
    ...
    for file in files:
        if file.endswith('.txt'):
            ...
            if 'CA' in file:
                ...
                počítání_ac +=1
                if počítání_ac == 4:
                    make_CA_graph(CA_data_files_list, data_dirs, labels)
                    print(len(CA_data_files_list))
                    print(CA_data_files_list)
                #počet_grafů +=1
                #print(počet_grafů)
                continue  
